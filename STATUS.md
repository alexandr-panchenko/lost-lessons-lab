...
| S3 — Structure, load, and destruction family | in progress | On 2026-07-21, the local gate passed 61 unit, 21 Worker/SQLite DO/R2 integration, build, and 15 Chromium tests; the direct `/structure` path remains unlinked pending production verification | The lowest-priority optional family must remain hidden until its complete vertical slice passes | Commit, deploy, and verify the isolated production path before enabling its practice entry | — |

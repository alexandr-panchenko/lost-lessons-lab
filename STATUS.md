...
| M5 — Complete hero and judge flow | in progress | Rescue Checkpoint R0 passed the complete local gate on 2026-07-21: format, lint, strict types, 65 unit, 22 Worker/SQLite DO/R2 integration, build, secret audit, and 22 Chromium tests. A focused verifier proved the dominant same-tab transition uses the existing room's real student capability, keeps Teacher/Student return control, preserves state, opens no new tab, filters teacher setup, places the task and Run control in the desktop viewport, and hides/redirects water, speed, and structure. Both 1280×900 frames were visually inspected. | The central bridge consequence still lacks the required visual impact and is explicitly deferred beyond R0. Production deployment and focused production verification are pending. | Commit, push, deploy, and production-verify R0; then stop without beginning R1 | Prior delivery through `892791e`; R0 commit pending |
| S1 — Water and volume family | cut | Implementation code and internal integration coverage remain; the R0 local public-route and navigation checks passed | Human review has not approved this scenario for the public product | Keep the production feature flag disabled | Historical implementation through `f436986`; hidden by R0 |
| S2 — Speed, time, and collision family | cut | Implementation code and internal integration coverage remain; the R0 local public-route and navigation checks passed | Human review has not approved this scenario for the public product | Keep the production feature flag disabled | Historical implementation through `fa857d3`; hidden by R0 |
| S3 — Structure, load, and destruction family | cut | Implementation code and internal integration coverage remain; the R0 local public-route and navigation checks passed | Human review has not approved this scenario for the public product | Keep the production feature flag disabled | Historical implementation through `5942254`; hidden by R0 |

...
| D-091 | 2026-07-21 | The final release candidate supports exactly one public lesson: fractions controlling a suspension bridge and maintenance-boat scene. | Human visual review selected the bridge-and-boat sketch and rejected the crane, reservoir, and all other scenario families. | Reuse only selected bridge experiment ideas; keep supporting implementation source disabled and remove every public claim or entry point that promises another family. |
| D-092 | 2026-07-21 | A fresh `/judge` room opens directly in the real student lesson while retaining a compact same-tab Student lesson / Teacher setup switch for the originating teacher capability. | The final judge path must begin with useful learner work rather than a prefilled transcript or link-copying step. | Initial state contains only goal, task, empty canvas, tools, submission, and a secondary sample-mistake loader; teacher and student share links remain available outside the judging path. |
| D-093 | 2026-07-21 | Supersede the `ctx.waitUntil` AI transport in D-051 with one Cloudflare Workflow instance per persisted attempt. | The final release-candidate requirement explicitly requires analysis to survive request/browser disconnect and a delayed response beyond 30 seconds. | The room retains its single-analysis lock and authoritative state; Workflow completion is idempotent, reload-safe, and terminal failure still exposes the bridge-specific manual fallback. |

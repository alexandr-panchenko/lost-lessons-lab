...
| Cloudflare deployment/version | S1 final `398a8f5c-9cc7-4887-9b85-b0f641cfbca4` from verification commit `f436986` | PASS through S1 |
...
| S2 — speed, time, and collision family | Local implementation in `src/shared/domain/speed.ts`, `src/shared/validation/speed-analysis.ts`, `src/simulations/speed/speed-world.ts`, `src/client/simulation/SpeedSimulation.tsx`, `fixtures/speed/packs.ts`, `tests/unit/speed.test.ts`, `tests/integration/speed-room.test.ts`, and `tests/e2e/speed.spec.ts`; production evidence target `docs/evidence/s2/speed-correct-and-collision.png` | On 2026-07-21, the pre-deployment local gate passed 55 unit, 19 Worker/SQLite DO/R2 integration, build, and 14 Chromium tests. The motion contract covered three curated packs, shared AI/manual validation, short/correct/overshoot/collision classifications, an extreme CCD case without tunneling, finite bounded bodies, explicit teardown, persistence, awards, replay, reload, and reset. Hero and water tests stayed green. Production and real-model verification are still required before this row can pass or the skill can be exposed. | PENDING |

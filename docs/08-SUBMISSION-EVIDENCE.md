...
| Cloudflare deployment/version | M5 final `3143b924-6be1-4436-aa62-167e4ef25e1d` from verification commit `892791e` | PASS through M5 |
...
| S1 — water and volume family | Local implementation in `src/shared/domain/water.ts`, `src/shared/validation/water-analysis.ts`, `src/simulations/water/water-world.ts`, `src/client/simulation/WaterSimulation.tsx`, `fixtures/water/packs.ts`, `tests/unit/water.test.ts`, `tests/integration/water-room.test.ts`, and `tests/e2e/water.spec.ts`; production evidence target `docs/evidence/s1/water-correct-and-overflow.png` | On 2026-07-21, the pre-deployment local gate passed 49 unit, 17 Worker/SQLite DO/R2 integration, build, and 13 Chromium tests. The water contract covered three curated packs, shared AI/manual validation, underfill/correct/overfill/overflow classifications, finite bounded bodies, settlement, explicit teardown, persistence, awards, replay, reload, and reset. The existing bridge hero tests stayed green. Production and real-model verification are still required before this row can pass or the skill chip can be exposed. | PENDING |

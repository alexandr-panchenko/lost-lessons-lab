...
| Cloudflare deployment/version | M2 final `b2b3b725-b3ab-48e5-8f80-8eedf46f6bb3` from delivery commit `c906082`; prior green rollback candidate `6926246a-2766-4c04-b246-5c4c10930444` | PASS for M2 |
...
| M3 — realtime canvas and deterministic bridge | `src/shared/canvas.ts`; `src/shared/domain/bridge.ts`; `src/client/canvas/`; `src/client/simulation/`; `src/simulations/bridge/bridge-world.ts`; `src/worker/room/RoomDurableObject.ts`; `tests/unit/canvas.test.ts`; `tests/unit/bridge.test.ts`; `tests/integration/canvas-room.test.ts`; `tests/e2e/bridge-manual.spec.ts`; `tests/e2e/realtime.spec.ts`; `tests/e2e/responsive.spec.ts`; `scripts/smoke-prod.ts` | On 2026-07-21, the local M3 gate passed 18 unit tests including bounded headless Planck outcomes, 9 Worker/SQLite DO integration tests, a production build, and 11 Chromium E2E tests. Two-context layers, explicit WebSocket disconnect/queued resend/no duplicate, immutable attempt cutoff, `4.08 m` recovery, `9 m` crossing, replay without API traffic, reload, three viewports, and reduced motion passed. Production commit/deployment evidence pending. | PENDING |

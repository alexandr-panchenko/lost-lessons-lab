...
| Safe comic failure | The physically legible catastrophe ends with a safe rescue beat | production frames/video and transcript | `docs/evidence/r2-1/wrong-bridge-failure.png`; `docs/evidence/r2-1/wrong-bridge-water-impact.png`; `docs/evidence/r2-1/wrong-bridge-complete-run.webm` | The inspected production capture shows deck peel, a rotating vehicle, broken sign/debris, deformed river surface, and the driver resurfacing in a rescue bubble | PASS technically; human review pending |
...
| Correction | Learner revises their own work or enters a directly solved fraction result; no prepared correct-answer control remains | browser trace and source | `src/client/App.tsx`; `src/client/simulation/ManualBridgePanel.tsx`; `tests/e2e/bridge-manual.spec.ts` | The production learner path offers Try again and progressive hints, accepts a direct fraction strategy without requiring decimal conversion, and reveals the full quarter/decimal explanation only after a correct submission | PASS |

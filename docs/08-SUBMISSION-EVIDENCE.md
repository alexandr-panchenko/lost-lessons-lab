...
| Judge URL | `https://lost-lessons-lab.sanocks.workers.dev/judge` | PASS through Rescue R2.1; human visual approval pending |
...
| Cloudflare deployment/version | Rescue R2.1 `bce1bb7b-26d0-4517-9a36-38df42dbc97b` from implementation `ec04eb9` plus performance correction `5818a60` | PASS technically; human visual approval pending |
...
| Final bridge release candidate | Branch `rescue/final-bridge-hero`; commit, Cloudflare version, CI, production screenshots/videos, and real GPT verification pending; `src/simulations/bridge/bridge-world.ts`; `src/client/simulation/BridgeSimulation.tsx`; `src/worker/workflow/AnalysisWorkflow.ts`; `tests/unit/bridge.test.ts`; `tests/workflow/analysis-workflow.test.ts`; `tests/e2e/room-shell.spec.ts`; `scripts/capture-final-bridge-rc.ts` | The 2026-07-21 local gate passed format, lint, strict types, 77 unit tests, 22 Worker/SQLite DO/R2 integration tests, a separate 31.19-second delayed Workflow test, build, tracked/history/bundle secret audit, and all 22 Chromium tests. Ten consecutive correct worlds reached the far-bank zone upright with zero bridge breaks, and ten consecutive wrong worlds preserved the ordered sag/snap/peel/release/boat/water sequence with bounded bodies and velocities. Production and human visual review remain pending. | PENDING production and human review |
...
| Correction | Learner receives a fresh editable work area; a secondary demo helper can load the correct handwriting sample only after the first wrong result | browser trace and source | `src/client/App.tsx`; `src/client/canvas/CanvasWorkspace.tsx`; `tests/e2e/bridge-manual.spec.ts` | The learner path offers Try again and progressive hints, accepts independent work, and reveals the full quarter/decimal explanation only after a correct submission | PASS locally; production pending |
...
| Text transcript | Simulation outcome is available as learner-readable text without validator or infrastructure diagnostics | DOM assertion and production verifier | `tests/e2e/bridge-manual.spec.ts`; `scripts/verify-rescue-r2.ts` | The post-run transcript states that the learner's `4.08 m` bridge bent, broke, hit the construction sign, entered the river, and ended safely; raw domain/validator/canvas-operation wording is absent | PASS |

...
| Rescue R2.1 — disclosure-safe staged wrong bridge | Pending implementation commit; `src/shared/learner-feedback.ts`; `src/worker/room/RoomDurableObject.ts`; `src/simulations/bridge/bridge-world.ts`; `src/client/simulation/BridgeSimulation.tsx`; `tests/unit/learner-feedback.test.ts`; `tests/unit/bridge.test.ts`; `tests/integration/analysis-room.test.ts`; `tests/e2e/bridge-manual.spec.ts`; `scripts/verify-rescue-r2.ts`; `scripts/capture-rescue-r2.ts` | The complete local gate passed format, lint, strict types, 74 unit tests, 22 Worker/SQLite DO/R2 integration tests, build, secret audit, and 22 Chromium tests. Tests prove the student capability omits hidden correct inputs, normalized answer leaks trigger curated replacement, learner DOM/accessibility labels omit `9 m` and `0.75` before a correct retry, hints disclose progressively, replay makes no new request, and reduced motion retains the ordered physical sequence. Physics contracts verify cable snap, sequential deck peel/release, forward travel, at least one full chassis rotation, sign destruction, detached vehicle part, bidirectional heightfield-wave propagation, and continuing aftermath without exact-coordinate assertions. Production deployment, capture, performance comparison, and human visual approval remain pending. | PENDING |

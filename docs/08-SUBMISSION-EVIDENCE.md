...
| Judge URL | `https://lost-lessons-lab.sanocks.workers.dev/judge` | PASS technically as final bridge RC; human visual approval pending |
...
| Cloudflare deployment/version | Final bridge RC `03f35408-6c66-4b75-be5a-3b2bd361b317` from deployed application commit `0347436` | PASS technically; human visual approval pending |
...
| Rescue R2.1 — disclosure-safe staged wrong bridge | Implementation `ec04eb9`; performance correction `5818a60`; CI `https://github.com/alexandr-panchenko/lost-lessons-lab/actions/runs/29849650131`; deploy/smoke `https://github.com/alexandr-panchenko/lost-lessons-lab/actions/runs/29849885170`; Cloudflare version `bce1bb7b-26d0-4517-9a36-38df42dbc97b`; `src/shared/learner-feedback.ts`; `src/worker/room/RoomDurableObject.ts`; `src/simulations/bridge/bridge-world.ts`; `src/client/simulation/BridgeSimulation.tsx`; `tests/unit/learner-feedback.test.ts`; `tests/unit/bridge.test.ts`; `tests/integration/analysis-room.test.ts`; `tests/e2e/bridge-manual.spec.ts`; `scripts/verify-rescue-r2.ts`; `docs/evidence/r2-1/` | The complete local and CI gates passed format, lint, strict types, 74 unit tests, 22 Worker/SQLite DO/R2 integration tests, build, secret audit, and 22 Chromium tests. Tests prove the student capability omits hidden correct inputs, normalized answer leaks trigger curated replacement, learner DOM/accessibility labels omit `9 m` and `0.75` before a correct retry, hints disclose progressively, replay makes no new request, and reduced motion retains the ordered physical sequence. Physics contracts verify cable snap, sequential deck peel/release, forward travel, at least one full chassis rotation, sign destruction, detached vehicle part, bidirectional heightfield-wave propagation, and continuing aftermath without exact-coordinate assertions. Focused verification against the exact gated production version observed the complete ordered sequence in 15.01 seconds at 41.5 fps average / 33.4 ms p95, versus R2's 28.6 fps average / 50.1 ms p95. The three 1440×620 production stage frames and 1440×1000, 25 fps, 21.4-second VP8 page-only capture were visually inspected as a contact sheet; no capability or personal content is visible. The product remains pending human visual approval. | PASS technically; human review pending |
| Final bridge release candidate | Branch `rescue/final-bridge-hero`; deployed application commit `0347436`; Cloudflare version `03f35408-6c66-4b75-be5a-3b2bd361b317`; `src/simulations/bridge/bridge-world.ts`; `src/client/simulation/BridgeSimulation.tsx`; `src/worker/workflow/AnalysisWorkflow.ts`; `tests/unit/bridge.test.ts`; `tests/workflow/analysis-workflow.test.ts`; `tests/e2e/room-shell.spec.ts`; `scripts/capture-final-bridge-rc.ts`; `docs/evidence/final-bridge-rc/` | The 2026-07-22 gate passed format, lint, strict types, 77 unit tests, 22 Worker/SQLite DO/R2 integration tests, a separate 31-second delayed/disconnected Workflow test, build, tracked/history/bundle secret audit, all 22 local Chromium paths, and all 22 production Chromium paths. Ten consecutive correct worlds reached the far-bank zone upright with no bridge break; ten consecutive wrong worlds preserved the ordered sag/snap/peel/release/boat/water sequence with bounded bodies and velocities. Two final production attempts both reported `gpt-5.6-sol`; replay made no new request, reload restored both, and reset completed without media errors. Final 1440×1000 production screenshots and three 25 fps WebM videos were visually inspected and contain no capability. The final resource-constrained capture measured 22.8 fps average / 50.1 ms p95 wrong and 24.6 fps average / 50.0 ms p95 correct; an immediately prior clean production pass of the identical renderer measured 28.3 / 26.2 fps. | PASS technically; human visual approval pending |
| S1 — water and volume family | Historical implementation evidence only. The source remains in the repository but is disabled by `PUBLIC_SUPPORTING_SCENARIOS_ENABLED=false` and absent from public routes, suggestions, chips, random selection, README, and JUDGING claims. | This family is not part of the final bridge release candidate and must not be represented as public or approved. | HIDDEN |
| S2 — speed, time, and collision family | Historical implementation evidence only. The source remains in the repository but is disabled by `PUBLIC_SUPPORTING_SCENARIOS_ENABLED=false` and absent from public routes, suggestions, chips, random selection, README, and JUDGING claims. | This family is not part of the final bridge release candidate and must not be represented as public or approved. | HIDDEN |
| S3 — structure, load, and destruction family | Historical implementation evidence only. The source remains in the repository but is disabled by `PUBLIC_SUPPORTING_SCENARIOS_ENABLED=false` and absent from public routes, suggestions, chips, random selection, README, and JUDGING claims. | This family is not part of the final bridge release candidate and must not be represented as public or approved. | HIDDEN |
...
| Wrong parameter controls physics | `4.08 m` creates actual short bridge geometry | source, physics test, production capture | `docs/evidence/r2-1/wrong-bridge-before-failure.png`; `tests/unit/bridge.test.ts` | The final production frame keeps the bridge at about 45% of the ravine, while bounded Planck contracts prove the later load, joint failures, collision, rotation, and water entry | PASS |
| Safe comic failure | The staged bridge-and-boat catastrophe ends with the vehicle resurfacing on bright emergency pontoons and the driver reacting in a flotation collar | release-candidate production frames/video and transcript | `docs/evidence/final-bridge-rc/wrong-climax.png`; `docs/evidence/final-bridge-rc/wrong-complete-run.webm` | The production frame/video were visually inspected: the short bridge folds downward, the rotating vehicle breaks the lightweight boat structure at center frame, and the driver resurfaces safely while the water and debris remain active | PASS technically; human visual approval pending |
| Answer-safe failure feedback | Before a correct retry, the learner sees their `4.08 m` result, mistaken `3/4 = 0.34` step, conceptual guidance, and progressive hints—but never `9 m` or `0.75` | capability-filtering integration test, guard unit test, DOM/accessibility E2E, production verifier | `src/shared/learner-feedback.ts`; `tests/unit/learner-feedback.test.ts`; `tests/integration/analysis-room.test.ts`; `tests/e2e/bridge-manual.spec.ts`; `scripts/verify-rescue-r2.ts` | Local/CI tests and focused production verification found neither hidden correct form in student transport/rendered/accessibility content before a correct retry; a leaking generated string is replaced with curated feedback | PASS |
| Causal explanation | Failure feedback identifies `3/4 = 0.34` as the step to revisit without disclosing the solution | screenshot/video and DOM assertion | `src/client/feed/LearnerBridgeFeedback.tsx`; `tests/e2e/bridge-manual.spec.ts` | The learner sees what happened, the written step to revisit, the fraction-as-division concept, and one next action; exact correct values appear only after a correct submission | PASS |
| Correction | Learner receives a fresh editable work area; a secondary demo helper can load the correct handwriting sample only after the first wrong result | browser trace and source | `src/client/App.tsx`; `src/client/canvas/CanvasWorkspace.tsx`; `tests/e2e/bridge-manual.spec.ts`; `docs/evidence/final-bridge-rc/retry-ui.png` | The production learner path appends the editable retry after the failed result, offers progressive hints, and reveals the full quarter/decimal explanation only after a correct submission | PASS |
...
| Reset | Reset returns the current judge task to its clean lesson start | E2E and production test | `tests/integration/reset-room.test.ts`; `scripts/smoke-prod.ts`; `scripts/verify-production-hero.ts` | Final production reset removed attempts, analyses, runs, awards, private media, and student ink while preserving the same room and task; attempt media unmounted before deletion, so the browser remained console-clean | PASS |
...
| Text transcript | Simulation outcome is available as learner-readable text without validator or infrastructure diagnostics | DOM assertion and production verifier | `tests/e2e/bridge-manual.spec.ts`; `scripts/verify-production-hero.ts` | The production post-run transcript states that the learner's `4.08 m` bridge sagged, lost a support cable, hit the maintenance boat canopy, entered the river, and resurfaced on emergency pontoons; raw domain/validator/canvas-operation wording is absent | PASS |
...
| Claim | Evidence needed | Status |
|---|---|---|
| First action is obvious | `docs/evidence/final-bridge-rc/initial-lesson.png`; production direct-entry browser test | PASS technically; human visual approval pending |
...
| The architecture can be extended without claiming unfinished public breadth | Shared template/domain contracts exist, while the current Build Week demo exposes only the bridge lesson | PASS |

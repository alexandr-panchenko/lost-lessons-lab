...
| Cloudflare deployment/version | M3 final `71f9ffc7-9423-413f-a52e-4ccac07ff6c4` from delivery fix `cb80021`; prior green M3 version `1923407c-8dc6-4cb9-b824-e3f72898b426` | PASS through M3 |
...
| M5 — complete hero and judge flow | Local candidate; `src/shared/judge-handwriting.ts`; `src/worker/domain/achievements.ts`; `src/worker/routes/reset.ts`; `src/client/feed/PreparedCorrectionPanel.tsx`; `src/client/feed/AchievementCard.tsx`; `tests/unit/achievements.test.ts`; `tests/integration/reset-room.test.ts`; `tests/e2e/bridge-manual.spec.ts` | On 2026-07-21, local validation passed 42 unit, 15 Worker/SQLite DO/R2 integration, build, and 12 Chromium tests. The full mocked/manual browser path covered editable prepared work, wrong physics, disaster award, replay without API traffic, prepared correction, correct physics, progress award, reload, and reset. The required live gate extracted both frozen wrong and correct samples exactly on first GPT-5.6 responses in 4.62s and 5.06s. Production deployment and literal real-AI judge execution remain pending. | IN PROGRESS |
...
| Replay | Wrong and correct scenes replay without OpenAI request | Playwright network assertion | `tests/e2e/bridge-manual.spec.ts`; production run on `cb80021` | Request count remained unchanged across Replay in the production browser run | PASS |
| Reset | Reset returns current judge task to fixture start | E2E and production test | report | Run button/path | PENDING |
| Persistence | Reload reconstructs visible feed and media | E2E plus production reload | `tests/e2e/bridge-manual.spec.ts`; production run on `cb80021` | Wrong and correct persisted run cards reappeared after reload | PASS through structured M3 runs |
...
| Requirement / criterion | Claim | Exact evidence target | URL / file / video timestamp | Verification | Status |
|---|---|---|---|---|---|
| Cloudflare deployment | React SPA and API run in one Worker | Worker config and deployed headers | `wrangler.jsonc`; Cloudflare version `71f9ffc7-9423-413f-a52e-4ccac07ff6c4` | Gated deploy and production smoke run `29804696808` passed | PASS |

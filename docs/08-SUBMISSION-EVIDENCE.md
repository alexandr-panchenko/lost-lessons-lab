...
| Teacher layer exclusion | Teacher annotations do not enter AI input | Unit raster fixture and intercepted/hashed E2E media | `tests/unit/canvas.test.ts`; `src/client/canvas/stroke-render.ts` | Unit test passed the same student-only cutoff function used for rasterization; AI request interception remains M4 evidence | PASS for raster boundary |
...
| Deterministic validation | Arithmetic and units are checked outside GPT | bridge domain tests and source | `src/shared/domain/bridge.ts`; `tests/unit/bridge.test.ts` | 18-test unit run classified hero values outside any model path | PASS for bridge/manual path |

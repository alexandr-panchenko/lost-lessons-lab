...
| S3 — structure, load, and destruction family | Core implementation pending commit; `src/shared/domain/structure.ts`; `src/shared/validation/structure-analysis.ts`; `src/simulations/structure/structure-world.ts`; `fixtures/structure/packs.ts`; `scripts/verify-production-structure.ts` | On 2026-07-21, the pre-deployment local gate passed 61 unit, 21 Worker/SQLite DO/R2 integration, build, and 15 Chromium tests. The direct structure route is deliberately unlinked until its real-model and production physics verification passes. | PENDING |

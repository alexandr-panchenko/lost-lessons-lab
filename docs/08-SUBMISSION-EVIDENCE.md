...
| M6 — reliability, fallbacks, and cost controls | Core implementation pending commit; `src/worker/media/attempt-media.ts`; `src/worker/security/logging.ts`; `src/worker/room/RoomDurableObject.ts`; `src/simulations/core/render-quality.ts`; `tests/unit/reliability-controls.test.ts`; `tests/integration/analysis-room.test.ts`; `tests/e2e/failure-recovery.spec.ts`; `scripts/verify-production-fallback.ts` | On 2026-07-21, the pre-deployment local gate passed 65 unit, 22 Worker/SQLite DO/R2 integration, build, and 18 Chromium tests. The opt-in wrong/correct live gate passed exact first responses in 5.16s and 4.45s with no repair. Production recovery and final judge-flow evidence remain pending. | PENDING |
